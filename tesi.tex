...
\newtcbox{\tag}{on line,
  colframe=gray,colback=lightgray,
  boxrule=0.5pt,arc=2pt,boxsep=0pt,left=2pt,right=2pt,top=1.5pt,bottom=0.5pt}
...
\begin{document}
	...
	For this reason, many of the conceptual vulnerabilities that a Tor exit node has can be applied to the Mysterium Network exit node. What makes Mysterium Network more vulnerable is the final use the users make of it.\\
	Tor is designed and principally used for critical situations such as for political, 

	(MAYBE DESCRIBE WHY MYSTERIUM AND TOR ARE DIFFERENCES)\\


	Figure X describes an end node configuration.\\
	...
	It uses the \tag{iptable} ``nfqueue extension'' for inline catching the packet and expressing a verdict.\\
...
\section{Conclusions}
	...
	Here there is an helpful consideration of how critical is an exit point for TOR.
